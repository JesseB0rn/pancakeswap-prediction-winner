...
export default function Main() {
  ...
  useEffect(() => {
    chrome.storage.sync.get(['privateKey', 'betAmount', 'logs']).then(({ privateKey, betAmount, logs }) => {
      try {
    ...
    <>
      ...
      <Box p="6" pt="0" mt="-30px" mb="12" w="lg-8">
        <HStack justify="center" mb="1">
          <Heading fontSize="4xl">PanWin</Heading>
        ...
        <HStack justify="space-between" mb="4" mt="6">
          <Button
            ...
            leftIcon={platforms[PLATFORMS.PancakeSwap] ? <CheckIcon /> : <IoSquareOutline />}
            onClick={() =>
              setPlatforms((p) => ({
                ...p,
          ...
          <Button
            ...
            leftIcon={platforms[PLATFORMS.CandleGenieBTC] ? <CheckIcon /> : <IoSquareOutline />}
            onClick={() =>
              setPlatforms((p) => ({
                ...p,
          ...
          <Button
            ...
            leftIcon={platforms[PLATFORMS.CandleGenieBNB] ? <CheckIcon /> : <IoSquareOutline />}
            onClick={() =>
              setPlatforms((p) => ({
                ...p,
          ...
          <Button
            ...
            leftIcon={platforms[PLATFORMS.CandleGenieETH] ? <CheckIcon /> : <IoSquareOutline />}
            onClick={() =>
              setPlatforms((p) => ({
                ...p,
        ...
        <Box mt="3" experimental_spaceY="2">
          ...
          <HStack mt="4">
            <Input placeholder="Private Key" type="password" w="full" value={privateKey} onChange={(e) => setPrivateKey(e.target.value)} />

            <Button
              colorScheme="brand"
              leftIcon={<LockIcon />}
              onClick={() => {
                chrome.storage.sync.set({ privateKey }).then(() => setFetchFlag.toggle())
              }}
            >
              Set
            </Button>
          ...
          <HStack>
            <InputGroup>
              ...
              <Input type="number" placeholder="BNB Amount" w="full" value={betAmount} onChange={(e) => setBetAmount(e.target.value)} />
            </InputGroup>
            <Button
              colorScheme="brand"
              leftIcon={<PlusSquareIcon />}
              onClick={() => {
                chrome.storage.sync.set({ betAmount }).then(() => setFetchFlag.toggle())
              }}
            >
              Set
            </Button>
              ...
              <Button
                onClick={async () => {
                  chrome.runtime.sendMessage({
                    ...
                    data: {
                      platforms,
                      strategy: strategy ? STRATEGIES.Experimental : STRATEGIES.Standard,
                    },
                  })
          ...
          <HStack justify="space-evenly" mb="8">
            <Text as="span" color="white" w="90" size="xl">
              <Link fontSize="xl" fontWeight={500} href="https://t.me/panwin" isExternal w="60" verticalAlign="bottom" textDecoration="underline">
                Telegram
              </Link>
            </Text>
            <Text as="span" color="white" w="90" size="xl">
              <Link fontSize="xl" fontWeight={500} href="https://discord.gg/panwin" isExternal w="60" verticalAlign="bottom" textDecoration="underline">
                Discord
              </Link>
            </Text>
          </HStack>
